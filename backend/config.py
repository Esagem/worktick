...
# What event title counts as "work"
WORK_EVENT_TITLE = os.environ.get("WORK_EVENT_TITLE", "McCrary Summer Work")

# Timezone (used for parsing all-day events; widgets currently don't depend on it)
TIMEZONE = ZoneInfo(os.environ.get("TIMEZONE", "America/Chicago"))

# Hourly wage in USD (gross). Widgets fetch this from /schedule and multiply
# elapsed-seconds * RATE / 3600 client-side.
HOURLY_RATE = float(os.environ.get("HOURLY_RATE", "0"))

# Database
DB_PATH = os.environ.get("DB_PATH", "/data/worktick.db")

# How often to poll Google Calendar. Schedule is stable → 6 hours is plenty.
...
# Calendar query window each poll. Generous lookback so all-time totals don't lose
